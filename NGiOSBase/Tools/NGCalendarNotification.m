...
#import "NGCalendarNotification.h"
#import "NGAlertView.h"

@implementation NGCalendarNotification
    ...
    if ([blockEventStore respondsToSelector:@selector(requestAccessToEntityType:completion:)]) {
        [blockEventStore requestAccessToEntityType:EKEntityTypeEvent completion:^(BOOL granted, NSError *error){
            dispatch_async(dispatch_get_main_queue(), ^{
                if (error) {
                    [NGAlertView alertWithMessage:error.localizedDescription andCompleteBlock:nil];
                } else if (!granted) { //被⽤户拒绝,不允许访问日历
                    [NGAlertView alertWithMessage:@"请设置允许访问日历~" andCompleteBlock:nil];
                } else {
                    if(startDate) {
