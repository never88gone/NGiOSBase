...
@implementation OTSNonModelLoadingView
...
{
	OTSNonModelLoadingView *loadingView = [[self alloc] initWithFrame:CGRectMake(0, 0, kWidth, kHeight)];
...
+ (void)hideLoadingForView:(UIView *)view animated:(BOOL)animated
{
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[OTSNonModelLoadingView class]]) {
			[viewsToRemove addObject:v];
		}
	}

    for (OTSNonModelLoadingView *loadingView in viewsToRemove){
		if (animated) {
