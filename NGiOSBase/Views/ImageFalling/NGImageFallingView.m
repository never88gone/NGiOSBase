...
#import "NGImageFallingView.h"
#import "NGSizeDefine.h"
#import "NGSize.h"
#import <SDWebImage/SDWebImageManager.h>
#import <SDWebImage/UIImageView+WebCache.h>
#define NGImageFallingElementViewWidth [NGSize getLengthWithSizeType:NGSizeType3_5 andLength:30]
...
- (void)willMoveToSuperview:(UIView *)newSuperview {
    if (self.fallingType <= ImageFallingTypeDownToUp) {
        if (newSuperview) {
            ...
            if ([superView isKindOfClass:[NGImageFallingView class]]) {
                NGImageFallingView *rainView = (id)superView;
        ...
        switch (self.fallingType) {
            case ImageFallingTypeLeftToRight:
                startCenter = CGPointMake(-NGImageFallingBigViewWidth/2, UI_CURRENT_SCREEN_HEIGHT/2);
                break;
            case ImageFallingTypeRightToLeft:
                startCenter = CGPointMake(UI_CURRENT_SCREEN_WIDTH+NGImageFallingBigViewWidth/2, UI_CURRENT_SCREEN_HEIGHT/2);
                break;
            default:
                break;
        }
...
-(void) animationMove
{
    ...
    switch (self.fallingType) {
        case ImageFallingTypeLeftToRight:
            startCenter = CGPointMake(-NGImageFallingBigViewWidth/2, UI_CURRENT_SCREEN_HEIGHT/2);
            endCenter = CGPointMake(UI_CURRENT_SCREEN_WIDTH+NGImageFallingBigViewWidth/2, UI_CURRENT_SCREEN_HEIGHT/2);
            break;
        case ImageFallingTypeRightToLeft:
            startCenter = CGPointMake(UI_CURRENT_SCREEN_WIDTH+NGImageFallingBigViewWidth/2, UI_CURRENT_SCREEN_HEIGHT/2);
            endCenter = CGPointMake(-NGImageFallingBigViewWidth/2, UI_CURRENT_SCREEN_HEIGHT/2);
            break;
        default:
            break;
    }
...
- (CGPathRef)p_randomBezierPath {
    ...
    if (self.fallingType == ImageFallingTypeDownToUp) {
        [path moveToPoint:CGPointMake(randomWdith, UI_CURRENT_SCREEN_HEIGHT+NGImageFallingElementViewWidth*2)];
        [path addLineToPoint:CGPointMake(randomWdith, -NGImageFallingElementViewWidth*2)];
    } else {
        [path moveToPoint:CGPointMake(randomWdith, -NGImageFallingElementViewWidth*2)];
        [path addLineToPoint:CGPointMake(randomWdith, UI_CURRENT_SCREEN_HEIGHT+NGImageFallingElementViewWidth*2)];
    }
    return path.CGPath; //设置路径
}

- (CGPoint)p_CGPointMake:(CGFloat)xPs yPs:(CGFloat)yPs {
    return CGPointMake([NGSize getLengthWithSizeType:NGSizeType3_5 andLength:xPs], [NGSize getLengthWithSizeType:NGSizeType3_5 andLength:yPs]);
}
...
@interface NGImageFallingView ()
@property (nonatomic, strong) CAEmitterLayer *emitterLayer;
@end

@implementation NGImageFallingView
{
...
- (void)startAnimation {
    ...
    if (self.fallingType <= ImageFallingTypeDownToUp) {
        ...
        NGImageFallingElementView *elementView = [[NGImageFallingElementView alloc] initWithFrame:CGRectMake(0, 0, NGImageFallingBigViewWidth, NGImageFallingBigViewHeight)];
...
- (NGImageFallingElementView *)p_dequeueImageFallingImgView {
    NGImageFallingElementView *aImageFallingImgView;
    if (_reuseFallingImgViewArray.count > 0) {
        ...
        aImageFallingImgView = [[NGImageFallingElementView alloc] initWithFrame:CGRectMake(-NGImageFallingElementViewWidth*2, -NGImageFallingElementViewWidth*2, NGImageFallingElementViewWidth, NGImageFallingElementViewWidth)];
...
- (void)p_popImageFallingImgView {
    NGImageFallingElementView *sprite = [self p_dequeueImageFallingImgView];
...
- (CAEmitterLayer *)p_fallingEmitterLayerWithRotate:(BOOL)isRotate image:(UIImage *)image {
    ...
    snowEmitter.emitterPosition = CGPointMake(UI_CURRENT_SCREEN_WIDTH / 2.0, -NGImageFallingElementViewWidth);
    ...
    snowflake.scale = NGImageFallingElementViewWidth / width / [[UIScreen mainScreen] scale];
