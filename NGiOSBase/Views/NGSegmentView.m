...
@interface NGSegmentView ()

@property (nonatomic, strong) NSMutableArray *btns;

@property (nonatomic, strong) NGSegmentBtnView *currentSelectedBtn;

@end

@implementation NGSegmentView
...
- (void)reloadData
{
    ...
    if (self.btns.count < count) {
        for (NSUInteger i = self.btns.count; i < count; i++) {
            NGSegmentBtnView *btn = [NGSegmentBtnView autolayoutView];
    ...
    [NGConstraintHelper setViews:self.btns equalToSuperview:self isHorizontal:YES];
    
    for (NGSegmentBtnView *view in self.btns) {
    ...
    [self.btns enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        STRONG_SELF;
        NGSegmentBtnView *btn = obj;
        btn.tag = idx;
        [self.delegate segment:self configSegmentBtn:btn atIndex:idx];
        [btn updateLabels];
        
        [NGBorder removeBorderWithView:btn type:NGBorderTypeAll];
        
        if (idx != 0) {
            [NGBorder addBorderWithView:btn type:NGBorderTypeLeft andColor:[UIColor redColor]];
        }
...
- (void)onPressedBtn:(NGSegmentBtnView *)sender
{
